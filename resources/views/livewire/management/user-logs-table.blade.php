                        ...
                        <tbody>
                                ...
                                <tr>
                                    <td colspan="4" class="px-6 py-10 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200">
                                        <div class="flex justify-center items-center gap-2 w-full">
                                            <flux:icon.book-text variant="mini" class="" />
                            ...
                            @else
                                ...
                                <tr class="bg-white dark:bg-zinc-900 hover:bg-gray-100 dark:hover:bg-neutral-700">
                                    ...
                                    <td
                                        class="px-4 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200 border-t border-b border-black/10 dark:border-white/10">
                                        @if ($logSession->unique_users_count > 0)
                                            <div class="flex items-center gap-1">
                                                <flux:icon.users variant="outline" class="w-4 h-4 text-gray-500 dark:text-neutral-400" />
                                                <span>{{ $logSession->unique_users_count }} {{ $logSession->unique_users_count == 1 ? 'user' : 'users' }}</span>
                                            </div>
                                        @else
                                                        ...
                                                        <flux:dropdown>
                                                            ...
                                                            <flux:menu>
                                                                <flux:menu.group heading="Paper Size">
                                                                    <flux:menu.item 
                                                                        wire:click="exportUserLogs({{ $logSession->id }}, 'A4')"
                                                                        wire:target="exportUserLogs({{ $logSession->id }}, 'A4')"
                                                                    >
                                                                        A4
                                                                    </flux:menu.item>
                                                                    <flux:menu.item 
                                                                        wire:click="exportUserLogs({{ $logSession->id }}, 'Letter')"
                                                                        wire:target="exportUserLogs({{ $logSession->id }}, 'Letter')"
                                                                    >
                                                                        Letter
                                                                    </flux:menu.item>
                                                                    <flux:menu.item 
                                                                        wire:click="exportUserLogs({{ $logSession->id }}, 'Legal')"
                                                                        wire:target="exportUserLogs({{ $logSession->id }}, 'Legal')"
                                                                    >
                                                                        Legal
                                                                    </flux:menu.item>
                                                                </flux:menu.group>
    ...
    <flux:modal name="remove-log-session" :dismissible="false"
        ...
        <div class="space-y-6">
            <div>
                ...
                <flux:text class="">
                    <span class="font-bold">{{ $logSessionDate ?? 'error' }}</span>, A.Y. {{ $logSessionSchoolYear ?? 'error' }}.
                </flux:text>
            </div>
                    ...
                    <div class="space-y-1">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            @if ($this->selectedLogRecords->isNotEmpty())
                                @php
                                    $uniqueUsersCount = $this->selectedLogRecords->filter(fn($r) => $r->loggable_type === 'student')->pluck('student_id')->unique()->count()
                                        + $this->selectedLogRecords->filter(fn($r) => $r->loggable_type === 'faculty')->pluck('faculty_id')->unique()->count();
                                @endphp
                                <div class="flex items-center gap-1 text-sm text-gray-500 dark:text-neutral-400">
                                    ...
                                    <span>{{ $uniqueUsersCount }} {{ $uniqueUsersCount == 1 ? 'user' : 'users' }}</span>
                                </div>
                            @endif
                        </div>
                    </div>
                                        ...
                                        <div class="space-y-0.5">
                                            ...
                                            <p class="font-semibold text-gray-900 dark:text-neutral-50">
                                                @if ($record->loggable_type === 'faculty')
                                                    @php $entity = $record->faculty; @endphp
                                                @else
                                                    @php $entity = $record->student; @endphp
                                                @endif
                                                @if ($entity)
