<div>
    <div class="flex flex-col bg-white dark:bg-zinc-900 border border-black/10 dark:border-white/10 [:where(&)]:p-4 [:where(&)]:rounded-lg">
        ...
        <div class="flex items-center justify-between mb-5">
            {{-- Search Input --}}
            @if ($courses->isNotEmpty() || !empty($search))
                <flux:input size="sm" icon="magnifying-glass" placeholder="Search courses..." class="max-w-100" wire:model.live.debounce.300ms="search" autocomplete="off" clearable />
            @endif
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
                                    ...
                                    <tr>
                                        <td colspan="3" class="px-6 py-10 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200">
                                            <div class="flex justify-center items-center gap-2 w-full">
                                                ...
                                                <flux:heading size="lg">No Courses Available</flux:heading>
                                            </div>
                                        </td>
                                    </tr>
                                @endif
                            ...
                            @else
                                ...
                                @foreach($courses as $course)
                                    <tr wire:key="{{ $course->id }}" class="hover:bg-gray-100 dark:hover:bg-neutral-700">
                                        <td class="px-3 py-3 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-neutral-200">
                                            <flux:badge size="sm" color="zinc">{{ $course->code }}</flux:badge>
                                        </td>
                                        ...
                                        <td class="px-3 py-3 whitespace-nowrap text-end text-sm font-medium">
                                            <div class="flex items-center justify-end">
                                                ...
                                                >
                                                    <flux:tooltip content="Delete Course" position="top">
                                                        <flux:icon.trash 
                                                            x-show="!isHovered" 
                                                            variant="outline" 
                                                    ...
                                                    <flux:tooltip content="Delete Course" position="top">
                                                        <flux:icon.trash 
                                                            ...
                                                            class="text-red-500 cursor-pointer ml-5" 
                                                            x-cloak 
                                                            wire:click="$dispatch('open-delete-course-modal', { id: {{ $course->id }} })"
                                                        />
                                                    </flux:tooltip>
                                                </div>
                                            </div>
